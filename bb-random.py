...
# 顺序
df = df_distance.iloc[:len(order.Index_order), :]
order.Index_order = order.Index_order.tolist()

# 给定参数         
r_dist_unit_cost = 10  # 单位距离的成本为10  
v_truck_speed = 60  # 车辆行驶速度为60 
lambda1_visiting_total_dist = 1  
lambda2_truck_total_cost = 1  
    ...
    def Expand(self):  # 生成能向下一层分支的节点list
        ...
        while i < len(truck.Index_truck):
            ...
            and self.T_truck[i] + df_distance.iloc[self.truck_last_order[i],order.Index_order[self.level + 1]]/v_truck_speed <= order.Time_latest[order.Index_order[self.level + 1]]:
                ...
                node_take.truck_last_order = copy.deepcopy(self.truck_last_order)  
...
#    print('--------------start')
    while heap:
        ...
        if node_to_expand.cost + c_min >= UB:  # 剪支
            ...
            node_list = node_to_expand.Expand()  # 分支后新加入的节点
            random.shuffle(node_list)  # node_list顺序随机!
    ...
    while optimal_node.Parent:  #调整sol格式
